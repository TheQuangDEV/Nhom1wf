...
namespace quanlynhanvien_cuahangmini
{
    ...
    {
        string strKetNoi = @"Data Source=LAPTOP-OFAAGIK4\SQLEXPRESS;Initial Catalog=QLCHMN;Integrated Security=True";
        ...
        private void btnSua_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(strKetNoi))
                {
                    ...
                    {
                        // Truyền dữ liệu y hệt như lúc Thêm
                        cmd.Parameters.AddWithValue("@TaiKhoan", txtTaiKhoan.Text);
                        cmd.Parameters.AddWithValue("@TenNhanVien", txtTenNhanVien.Text);
                        cmd.Parameters.AddWithValue("@NgaySinh", dtpNgaySinh.Value.Date);
                        cmd.Parameters.AddWithValue("@DiaChi", cboDiaChi.Text);
                        ...
                        if (soDongBiSua > 0)
                        {
                            ...
                            LoadDuLieuTuSQL(); // Tải lại bảng để thấy thay đổi ngay
                        }
                        else
                        {
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            ...
            if (xacNhan == DialogResult.Yes)
            {
                ...
                try
                {
                    ...
                    LoadDuLieuTuSQL(); // Tải lại bảng để thấy dòng đó đã bốc hơi

                    // Nếu bạn có hàm ResetThongTin() thì gọi ở đây để xóa trắng các ô text
                    ResetThongTin(); 
                }
                catch (Exception ex)
                {
